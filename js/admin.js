/* =====================================================================
   ÉPARGNE PLUS — Admin dashboard controller (sécurisé)
   ---------------------------------------------------------------------
   CORRECTIONS :
   [1] Hashage PBKDF2 lors de l'ajout/modification d'un membre
   [2] Politique de mot de passe appliquée côté admin
   [3] Champ mot de passe masqué par défaut dans le formulaire membre
   [4] Validation stricte des entrées avant envoi au DB
   ===================================================================== */
(function () {
  ...
  function renderDashboard() {
    ...
    grid.innerHTML = cards.map(c => `
      <div class="stat-card glass">
        <span class="glow"></span><div class="ico ${c.ico}">${c.icon}</div>
        <div class="label">${escapeHtml(c.label)}</div>
        ${c.custom !== undefined
          ? `<div class="value">${escapeHtml(c.custom)}</div><div class="trend text-muted">${escapeHtml(c.subCustom)}</div>`
          : `<div class="value" data-count="${c.value}" data-money="${c.money?1:0}" data-suffix="${escapeHtml(c.suffix||"")}">0</div>${c.sub ? `<div class="trend text-success">${escapeHtml(c.sub)}</div>` : ""}`}
      </div>`).join("");
    initCounters(grid);
  ...
  /* ============================================================
     MEMBERS — avec hashage PBKDF2 pour les mots de passe
     ============================================================ */
  let membersState = { page:1, perPage:6, search:"" };
  const memberForm = document.getElementById("memberForm");

  /* Ajouter un champ password de type password (pas text) */
  const pwdField = document.getElementById("f_password");
  if (pwdField) pwdField.type = "password";

  function fillMemberForm(m) {
    document.getElementById("memberId").value       = m ? m.id : "";
    document.getElementById("f_lastName").value    = m ? escapeHtml(m.lastName)  : "";
    document.getElementById("f_firstName").value   = m ? escapeHtml(m.firstName) : "";
    document.getElementById("f_gender").value      = m ? m.gender : "F";
    document.getElementById("f_phone").value       = m ? escapeHtml(m.phone)   : "";
    document.getElementById("f_address").value     = m ? escapeHtml(m.address) : "";
    document.getElementById("f_login").value       = m ? escapeHtml(m.login)   : "";
    document.getElementById("f_password").value    = "";  // ne jamais pré-remplir le mot de passe
  ...
  memberForm.addEventListener("submit", async (e) => {
    ...
    const login     = sanitizeLogin(document.getElementById("f_login").value.trim());
    const password  = document.getElementById("f_password").value;

    if (!lastName || !firstName || !login) {
      toast("Champs obligatoires manquants", "warn"); return;
    }

    const dupe = DB.members().find(m => m.login === login && m.id !== id);
    ...
    if (id) {
      ...
      const patch = { lastName, firstName, gender, phone, address };
      if (password) {
        const pwdErr = PWD_POLICY.test(password);
        if (pwdErr) { toast(pwdErr, "warn"); return; }
        submitBtn.disabled = true; submitBtn.textContent = "Hashage…";
        patch.pwdHash = await Crypto.hashPassword(password);
      }
      DB.updateMember(id, patch);
      ...
      DB.addMember({ lastName, firstName, gender, phone, address, login, pwdHash });
      toast("Membre ajouté", "success");
    }

    submitBtn.disabled = false;
    submitBtn.innerHTML = id ? "💾 Enregistrer" : "＋ Ajouter";
    fillMemberForm(null);
    renderMembers();
  });
  ...
  document.getElementById("memberSearch").addEventListener("input", debounce((e) => {
    membersState.search = e.target.value.toLowerCase().trim();
    membersState.page   = 1;
    renderMembers();
  }, 180));
  ...
  function renderMembers() {
    ...
    document.getElementById("membersTable").innerHTML = `
      ...
      <tbody>${pageItems.map(m => { const t = memberTotals(m.id); return `
        <tr>
          <td class="avatar-cell">${avatarHTML(m)}<div>
            <div style="font-weight:600">${escapeHtml(m.firstName)} ${escapeHtml(m.lastName)}</div>
            <small class="text-muted">${escapeHtml(m.login)} · ${m.gender==="F"?"♀":"♂"}</small>
          </div></td>
          ...
          <td><div class="row-actions">
            <button class="icon-btn" title="Modifier" data-edit="${escapeHtml(m.id)}">✏️</button>
            <button class="icon-btn" title="Supprimer" data-del="${escapeHtml(m.id)}">🗑️</button>
          </div></td>
    ...
    document.querySelectorAll("#membersTable [data-edit]").forEach(b =>
      b.addEventListener("click", () => { fillMemberForm(DB.member(b.dataset.edit)); window.scrollTo({ top:0, behavior:"smooth" }); }));
    document.querySelectorAll("#membersTable [data-del]").forEach(b =>
      b.addEventListener("click", () => {
        const m = DB.member(b.dataset.del);
        if (!m) return;
        modal({ title:"Supprimer le membre", body:`Supprimer <b>${escapeHtml(m.firstName)} ${escapeHtml(m.lastName)}</b> et tous ses dépôts ? Cette action est irréversible.`,
          confirmText:"Supprimer", danger:true, onConfirm: () => { DB.deleteMember(m.id); toast("Membre supprimé","success"); renderMembers(); } });
      }));
  }
  ...
  function populateMemberSelect(sel, includeAll) {
    ...
    sel.innerHTML = (includeAll ? `<option value="all">Tous les membres</option>` : `<option value="" disabled selected>— Choisir un membre —</option>`)
      + members.map(m => `<option value="${escapeHtml(m.id)}">${escapeHtml(m.firstName)} ${escapeHtml(m.lastName)} (${escapeHtml(m.login)})</option>`).join("");
  }
  ...
  function historyFiltered() {
    ...
    list.sort((a,b) => {
      if (k === "name") { const av = DB.member(a.memberId)?.firstName||"", bv = DB.member(b.memberId)?.firstName||""; return av.localeCompare(bv)*histState.sortDir; }
      return (a[k]-b[k])*histState.sortDir;
    });
    return list;
  }
  ...
  function renderHistory() {
    ...
    document.getElementById("historyTable").innerHTML = `
      <thead><tr>
        <th class="sortable" data-sort="ts">Date${arrow("ts")}</th><th>Heure</th>
  ...
  function renderReports() {
    ...
    document.getElementById("reportCards").innerHTML = defs.map(d => { const r = make(d.k); return `
      ...
      <div class="label">Rapport ${escapeHtml(d.t)}</div>
  ...
  function buildReportTable() {
    ...
    document.getElementById("reportSummary").innerHTML = `
      <div class="info-list">
        <div class="info-item"><small>Période</small><b>${escapeHtml(labels[type])}</b></div>
  ...
  function renderStats() {
    Charts.trend("chartTrend",14); Charts.byUser("chartByUser");
    Charts.amounts("chartAmounts"); Charts.monthly("chartMonthly");
  }

  /* ============================================================
     SETTINGS — changement de mot de passe admin avec PBKDF2
     ============================================================ */
  ...
  document.getElementById("pwdForm").addEventListener("submit", async (e) => {
    ...
    const pwdErr = PWD_POLICY.test(p1);
    if (pwdErr) { toast(pwdErr,"warn"); return; }
    if (p1 !== p2) { toast("Les mots de passe ne correspondent pas","error"); return; }
    const btn = e.target.querySelector("button[type=submit]");
    btn.disabled = true; btn.textContent = "Hashage…";
    const pwdHash = await Crypto.hashPassword(p1);
    DB.updateAdmin({ pwdHash }); DB.log("admin","Mot de passe administrateur modifié");
  ...
  document.getElementById("resetDataBtn").addEventListener("click", () => {
    modal({ title:"Charger les données de démonstration", body:"Les membres et dépôts seront remplacés. Votre compte administrateur sera conservé. Continuer ?",
      confirmText:"Charger la démo", danger:true, onConfirm: () => { DB.reset(); toast("Données de démonstration chargées","success"); go("dashboard"); } });
  });
